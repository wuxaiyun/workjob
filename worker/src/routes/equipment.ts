...
export function equipmentRoutes(): Hono<AppEnv> {
  ...
  app.post('/', async (c) => {
    ...
    for (const key of ['original_value', 'net_value'] as const) {
      if (editable[key] === '') editable[key] = null;
      else if (editable[key] !== undefined) editable[key] = Number(editable[key]);
    }
  ...
  app.patch('/:tag_no/status', adminOnly(), async (c) => {
    ...
    const row = await c.env.DB.prepare(
      `SELECT id, status FROM equipment WHERE tag_no = ? AND deleted_at IS NULL`
    )
      .bind(tagNo)
    ...
    await c.env.DB.prepare(
      `INSERT INTO operation_log (operator, action, target_table, target_key, old_value, new_value)
       VALUES (?, '状态变更', 'equipment', ?, ?, ?)`
    )
      .bind(user.username, tagNo, row.status, status)
      .run();
